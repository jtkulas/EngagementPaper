---
title             : "Engagement vs. Attitude: Testing Measurement Invariance across Item Orderings"
...
Our 18-item engagement measure was crafted to be intentionally complex (each item loads on two constructs). This complexity, however, derives from a crossing of the attitudinal components of affect, cognition, and behavior with the substantive engagement components of vigor, dedication, and absorption. Within the current investigation, we realized $\alpha$'s of `r round(abs$total$raw_alpha,2)` (Absorption), `r round(ded$total$raw_alpha,2)` (Dedication), `r round(vig$total$raw_alpha,2)` (Vigor), `r round(aff$total$raw_alpha,2)` (Affect), `r round(cog$total$raw_alpha,2)` (Cognition), and `r round(beh$total$raw_alpha,2)` (Behavior).
...
### Item order conditions

The order of item presentation was: 1) random within substantive dimension, 2) random within substantive dimension, 3) parcels of substantive within attitudinal (greater attitudinal context), 4) parcels of attitudinal within substantive (greater substantive context), 5) parcels of substantive within attitudinal (limited attitudinal context), and 6) parcels of attitudinal within substantive (limited substantive context). For example, in condition 1, the first items presented were, for example, all Affect. Once this item list was fully exhausted, the respondent was administered Behavioral items, and once these were completed the respondent was then administered the Cognitive item set[^yaddup].  We view these orderings as cues regarding factor structure, and anticipate empirical factor structures to reflect these cues.
